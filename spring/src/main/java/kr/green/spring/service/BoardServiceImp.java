...
public class BoardServiceImp implements BoardService {
	...
	public int updateBoard(BoardVO board, MultipartFile[] file, Integer[] fileNum) {
		...
		//fileList와 fileNum의 첨부파일을 비교하여 fileList에서 fileNum으로 넘어오지 않은 파일을 삭제
	...
	@Override
	public ResponseEntity<byte[]> downloardFile(String fileName) throws IOException {
	...
	// 서버에 업로드된 파일을 삭제 후, DB에 삭제하는 메소드
	...
	public int updateRecommend(MemberVO user, int board, int state) {
		if(user == null)
				return -1;
		// 게시글 번호와 유저 아이디를 추천/비추천한 적이 있는지 확인함
		...
		// 있으면 
			...
			// 기존 state 정보와 누른 버튼 정보를 비교히여 같으면 state를 0으로 수정하고 0을 리턴(추천 누르고 추천, 비추천 누르고 비추천)
