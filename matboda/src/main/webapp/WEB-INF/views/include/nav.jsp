...
.nav .user-menu .me_nickname {
	font-size: 20px;
	text-align: center;
}

.nav .user-menu .mypage a, .nav .user-menu .signout a {
	width: 100px;
	height: 50px;
	padding: 10px;
	background-color: white;
	...
	<div class="nav-container">
		<div class="nav">
			...
			<div class="logo logo-long">
				<a href="<%=request.getContextPath()%>/"><h2>맛집 정보 다모아</h2></a>
			</div>
			<div class="search">
				<form action="<%=request.getContextPath()%>/restaurant/search">
					<input class="search-input" type="text" name="search"
						value="<c:out value="${pm.criteria.search}"/>"
						placeholder="지역이나 식당이름">
			...
			<div class="user-box">
				<c:if test="${user == null}">
					<div class="signin">
						<a href="<%=request.getContextPath()%>/member/signin"><i
				...
				<c:if test="${user != null}">
					<div class="account">
						<c:choose>
							<c:when
								test="${user.me_picture == null || user.me_picture == ''}">
								<img
									src="/matboda/img/2021/08/30/b085dc96-3945-40fb-b974-eeed6408cf27_img.png">
							</c:when>
							<c:otherwise>
