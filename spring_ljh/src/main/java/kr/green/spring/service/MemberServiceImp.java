...
public class MemberServiceImp implements MemberService {
	...
	public MemberVO signin(MemberVO user) {
		...
		// DAO에게 아이디를 알려 주면서 회원정보를 가져오라고 시킴
		MemberVO dbUser = memberDao.getMember(user.getId());
		// 가져온 회원정보와 비밀번호를 확인하여 일치하면 회원정보를 반환하고 일치하지 않으면 null을 반환
	...
	public int updateMember(MemberVO user) {
		...
		// 다오에게 아이디를 주면서 기존의 회원 정보를 가져오라고 시킴
		if(user == null) {
			return 0;
		}
		MemberVO dbUser = memberDao.getMember(user.getId());
		...
		// 수정할 회원 정보의 비밀번호가 있으면, 기존 회원 정보의 비밀번호를 변경
